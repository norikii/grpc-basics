...
func rateLaptop(laptopClient pb.LaptopServiceClient, laptopIDs []string, scores []float64) error {
	...
	if err != nil {
		return fmt.Errorf("cannto rate laptop: %v", err)
	}

	waitResposne := make(chan error)
	go func() {
		res, err := stream.Recv()
		if err == io.EOF {
...
func uploadImage(laptopClient pb.LaptopServiceClient,laptopID string, imagePath string) {
	...
	for {
		...
		if err != nil {
			log.Fatal("cannot read a chuck to the buffer: ", err)
		}
		...
		err = stream.Send(req)
		if err != nil {
			log.Fatal("cannot send chuck to server: ", err, stream.RecvMsg(nil))
		}
	}

	res, err := stream.CloseAndRecv()
	if err != nil {
		log.Fatal("cannot receive responses: ", err)
	}
